...
export default function DashboardPage() {
  ...
  async function saveSettings() {
    ...
    try {
      ...
      if (settings?.id) {
        await db.transact(db.tx.siceosSettings[settings.id].update(payload));
      } else {
  ...
  async function runScan() {
    ...
    addLog(
      `Perfil: ${settings.companyType} · ${
        (settings.keywords as string[])?.length || 0
      } palabras clave`
    );
    try {
      addLog("Conectando con motor de IA...");
      ...
      if (!resp.ok) {
        ...
        throw new Error((errData as { error?: string }).error || `Error ${resp.status}`);
      }
      const data = await resp.json() as ScanResult;
      addLog(
        `✓ Escaneo completado · ${data.tenders?.length || 0} licitaciones encontradas`
      );
      setResult(data);
    } catch (e: unknown) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          {isAdmin && (
            <a
              href="/admin"
              style={{ display: "flex", alignItems: "center", gap: "6px", color: "var(--warning)", fontSize: "0.8125rem", textDecoration: "none" }}
            >
              <Shield size={14} />
              Admin
            </a>
      ...
      <main style={{ padding: "32px 24px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          <div>
            <div style={{ color: "var(--muted)", fontSize: "0.75rem", letterSpacing: "0.08em", marginBottom: "6px" }}>
              PERFIL ACTIVO
            </div>
            {settings ? (
              <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                ...
                {settings.department && (
                  <span style={{ color: "var(--muted)", fontSize: "0.875rem" }}>· {settings.department as string}</span>
                )}
          ...
          >
            {scanning ? (
              <>
                <Radar size={16} style={{ animation: "spin 1s linear infinite" }} />
                Escaneando...
              </>
            ) : (
              <>
          ...
          <div className="terminal" style={{ marginBottom: "24px" }}>
            {logs.map((log, i) => (
              <div key={i} className="log-line">
                <span className={log.includes("✓") ? "log-ok" : log.includes("✗") ? "log-err" : ""}>
                  {log}
                </span>
              </div>
            ))}
            {scanning && (
              <div className="log-line">
                <span style={{ color: "var(--accent)" }}>▋<span className="cursor-blink">█</span></span>
              </div>
            )}
          </div>
        )}
        ...
        {result && (
          <>
            {result.summary && (
              <div
                className="card"
                style={{ padding: "16px 20px", marginBottom: "20px", borderLeft: "3px solid var(--accent)", fontSize: "0.875rem", lineHeight: "1.6" }}
              >
                <div style={{ color: "var(--accent)", fontSize: "0.75rem", marginBottom: "8px", letterSpacing: "0.05em" }}>
                  RESUMEN DE IA
                </div>
                {result.summary}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
              ...
              <span style={{ color: "var(--muted)", fontSize: "0.75rem", display: "flex", alignItems: "center", gap: "4px" }}>
                <Clock size={12} />
                {new Date(result.scannedAt).toLocaleString("es-BO")}
              </span>
            </div>
                          ...
                          <div style={{ width: "60px", height: "4px", background: "var(--border)", borderRadius: "2px", overflow: "hidden" }}>
                            <div
                              style={{
                                width: `${t.relevancia}%`,
                                height: "100%",
                                background: t.relevancia >= 70 ? "var(--success)" : t.relevancia >= 40 ? "var(--warning)" : "var(--muted)",
                              }}
                            />
                          </div>
        ...
        {!scanning && !result && logs.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 24px", color: "var(--muted)" }}>
            ...
            <p style={{ fontSize: "0.875rem" }}>
              {settings ? 'Presioná "Escanear SICOES" para iniciar' : "Configurá tu perfil primero"}
            </p>
          </div>
        )}
      </main>
        ...
        >
          <div className="card" style={{ width: "100%", maxWidth: "600px", maxHeight: "90vh", overflowY: "auto", padding: "32px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
              ...
              <button onClick={() => setShowSettings(false)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer" }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ marginBottom: "24px" }}>
              <label style={{ display: "block", fontSize: "0.75rem", color: "var(--muted)", marginBottom: "12px", letterSpacing: "0.05em" }}>
                TIPO DE EMPRESA
              </label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {COMPANY_TYPES.map((ct) => (
                  <button
                    key={ct.value}
                    type="button"
                    onClick={() => applyPreset(ct.value)}
                    style={{
                      padding: "8px 16px",
                      borderRadius: "6px",
                      border: `1px solid ${formCompanyType === ct.value ? "var(--accent)" : "var(--border)"}`,
                      background: formCompanyType === ct.value ? "rgba(0,229,195,0.1)" : "transparent",
                      color: formCompanyType === ct.value ? "var(--accent)" : "var(--text)",
                      fontSize: "0.875rem",
                      cursor: "pointer",
                      fontFamily: "inherit",
                    }}
            ...
            <div style={{ marginBottom: "24px" }}>
              <label style={{ display: "block", fontSize: "0.75rem", color: "var(--muted)", marginBottom: "8px", letterSpacing: "0.05em" }}>
                PALABRAS CLAVE ({formKeywords.length})
              </label>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "12px" }}>
                {KEYWORD_PRESETS[formCompanyType].map((kw) => (
                  <button
                    key={kw}
                    type="button"
                    className={`chip ${formKeywords.includes(kw) ? "active" : ""}`}
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                />
                <button className="btn-ghost" onClick={addCustomKeyword} style={{ padding: "10px 14px", flexShrink: 0 }}>
                  <Plus size={16} />
                </button>
              </div>
              {formKeywords.filter((kw) => !KEYWORD_PRESETS[formCompanyType].includes(kw)).length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "8px" }}>
                  {formKeywords
                    .filter((kw) => !KEYWORD_PRESETS[formCompanyType].includes(kw))
                    .map((kw) => (
                      <button key={kw} type="button" className="chip active" onClick={() => toggleKeyword(kw)}>
                        ✕ {kw}
                      </button>
                    ))}
                </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px", background: "var(--bg2)", borderRadius: "8px", marginBottom: "24px" }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${formNotify ? "var(--success)" : "var(--border)"}`,
                  background: formNotify ? "rgba(0,214,143,0.1)" : "transparent",
                  borderRadius: "6px",
                  color: formNotify ? "var(--success)" : "var(--muted)",
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button className="btn-primary" onClick={saveSettings} disabled={isSaving} style={{ flex: 1 }}>
        ...
        >
          <div className="card" style={{ width: "100%", maxWidth: "620px", maxHeight: "90vh", overflowY: "auto", padding: "32px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
              ...
              <button onClick={() => setSelectedTender(null)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", flexShrink: 0, marginLeft: "16px" }}>
                <X size={20} />
              </button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
              {[
                { label: "ENTIDAD", value: selectedTender.entidad },
                { label: "MONTO", value: selectedTender.monto || "No especificado" },
                { label: "FECHA", value: selectedTender.fecha || "—" },
                { label: "DEPARTAMENTO", value: selectedTender.departamento || "—" },
              ].map(({ label, value }) => (
            ...
            <div style={{ marginBottom: "20px" }}>
              <div style={{ color: "var(--muted)", fontSize: "0.75rem", letterSpacing: "0.08em", marginBottom: "8px" }}>
                ANÁLISIS DE RELEVANCIA · {selectedTender.relevancia}%
              </div>
              <div style={{ height: "6px", background: "var(--border)", borderRadius: "3px", overflow: "hidden", marginBottom: "10px" }}>
                <div
                  style={{
                    width: `${selectedTender.relevancia}%`,
                    height: "100%",
                    background: selectedTender.relevancia >= 70 ? "var(--success)" : selectedTender.relevancia >= 40 ? "var(--warning)" : "var(--muted)",
                  }}
                />
              </div>
